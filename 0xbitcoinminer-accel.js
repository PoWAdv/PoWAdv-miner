...
const BLOCK_REWARD = 25000000000;
...
module.exports =  {
  ...
  //  async init(web3, subsystem_command, vault, networkInterface, miningLogger)
    ...
    async mine(subsystem_command,subsystem_option)
    {
	  ...
	  // POWA
	  //this.balance = await this.vault.getEtherBalance(this.minerEthAddress);
  ...
  //    console.log('collect parameters.. ')
      ...
      try
      {
		  ...
		  miningParameters.tokenbalance = parameters.tokenbalance;
		  miningParameters.miningDifficulty = parameters.miningDifficulty;
		  
		  
		  if(miningParameters.tokenbalance > BLOCK_REWARD*2)
		  {  
		  var self_minted_blocks = Math.floor( parameters.tokenbalance / BLOCK_REWARD);
		  
		//  console.log('Self minted:'+ self_minted_blocks); 
		  
		  var discount_mod = self_minted_blocks*self_minted_blocks;
		  
		//  console.log('Self divider:'+ discount_mod); 
		  
		  
          miningParameters.miningDifficulty = Math.floor(parameters.miningDifficulty / discount_mod);
		  }
		  
		  if(miningParameters.miningDifficulty<1)
			  miningParameters.miningDifficulty = 1;
            ...
            if(this.miningTarget  == null || !this.miningTarget.eq(miningParameters.miningTarget   ) )
            {
             ...
             if(this.miningDifficulty != miningParameters.miningDifficulty)
             {
              ...
              this.miningDifficulty = miningParameters.miningDifficulty
               ...
               console.log("New difficulty(PoS): " + this.miningDifficulty);
             }
