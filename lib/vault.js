  ...
  async requirePassword(require)
  {
    ...
    async init(web3,miningLogger)
    {
     ...
     if( requirePassword && userPassword == null ){
      ...
      localStorage = new LocalStorage(this.get0xBitcoinLocalFolderPath()+'/PoWAdvdatas');
    ...
    {
      return this.getOSLocalDataFolderPath()  + '/.PoWAdv';
    },
   ...
   async handleAccountCommand(subsystem_command,subsystem_option)
	...
	async getTokenBalance2(address)
    {
     // var decimals = await this.tokenContract.methods.decimals().call()

      var balanceTokensRaw2 =  await this.tokenContract.methods.balanceOf(address).call();

      return balanceTokensRaw2.toString();
    },
    ...
    encryptedStorageExists()
    {
      ...
      if (!fs.existsSync(this.get0xBitcoinLocalFolderPath()) || !fs.existsSync(this.get0xBitcoinLocalFolderPath() +'/PoWAdvdatas' )  ){
        return false;
      }

      if (!fs.existsSync(this.get0xBitcoinLocalFolderPath()+'/PoWAdvdatas/vaultdata' ) ){
        return false;
      }
